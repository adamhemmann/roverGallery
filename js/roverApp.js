...
function repopulateCameraSelect() {
  ...
  if (workingManifest.photos[solVal] == null) {
    ...
    $("#cameraSelect")
      .prop("disabled", true)
      .html("<option>No images for this rover/sol combination.</option>");
  ...
  if (workingManifest.photos[solVal] != null) {
    ...
    cameras.forEach((cam) => {
      selectString +=
        "<option value='" + cam + "' value='" + cam + "'>" + cam + "</option>";
    });
...
function getOpportunityManifest() {
  ...
  xmlHttp.open(
    "GET",
    "https://api.nasa.gov/mars-photos/api/v1/manifests/opportunity?api_key=" +
      apiKey,
    true
  );
  xmlHttp.send();
...
function getSpiritManifest() {
  ...
  xmlHttp.open(
    "GET",
    "https://api.nasa.gov/mars-photos/api/v1/manifests/spirit?api_key=" +
      apiKey,
    true
  );
  xmlHttp.send();
  ...
  if (!(solVal in workingManifest.photos)) {
    alert(
      workingManifest.name +
        " does not contain any photos for the specified sol.\nPlease select a different sol."
    );
    return false;
  } else {
    return true;
...
function showDivs(n) {
  ...
  for (i = 0; i < x.length; i++) {
    ...
    $("#currentSlideRoverName").html(
      "<b>Rover Name:</b> " +
        currentPhotoObj.rover.name +
        "<br><b>Camera Name:</b> " +
        currentPhotoObj.camera.full_name +
        "<br><b>Rover Left Earth:</b> " +
        currentPhotoObj.rover.launch_date +
        "<br><b>Rover Landed on Mars:</b> " +
        currentPhotoObj.rover.landing_date +
        "<br><b>This Photo Was Taken:</b> " +
        currentPhotoObj.earth_date
    );
  }
